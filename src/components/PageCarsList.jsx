...
/*const CarsLine = ({ car }) =><div>
                                    <input type="text" value={car.registration_number} onChange={this.props.onChange(key)}/>
                                    <input type="text" value={car.brand}/>
                                    <input type="text" value={car.model}/>
                                    <input type="text"  value={car.location}/>
                                    <input type="text"  value={car.renting_company}/>
                                    <input type="number" value={car.price}/>
                                    <input type="number" value={car.year_of_production}/>
                                    <button>edit/delete</button>
                                    <button>show avaliability</button>
                              </div>*/
class PageCarsList extends React.Component {
  ...
  onEditClick(i){
    this.setState({id:this.props.cars[i].id,
  ...
  onSaveChangedClick(){
   ...
   //    this.setState({isSaving:true});
     fetch('http://localhost:3004/cars/'+this.state.id, {
       method: 'PUT', 
       headers: {
         'Content-Type': 'application/json',
         'Accept': 'application/json'
 ...
 // }
  ...
  if(this.props.loading){return <p>Loading ...</p> }
    ...
    return (
      ...
      {/* <div id="searchPanel">
          <input id="searchInput" placeholder="Search..." onChange={this.onChange_search}/>
          <button id="searchButton" onClick={() => this.props.onSearchclick(this.state.search)}>search</button>
        </div>*/}
        <div id="listHeader">  
...
const mapDispatchToProps = (dispatch) => ({
 ...
 //saveNewCar:(car)=>dispatch(saveNewCar(car)),
